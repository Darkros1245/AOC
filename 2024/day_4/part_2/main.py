...
def get_neighbors(
    ...
    return [
        ...
        if 0 <= x + _x < len(board[0]) and 0 <= y + _y < len(board)
    ]


# Get neighbors below and to the right
def get_cross_neighbors(
    x: int, y: int, board: list[str]
) -> list[tuple[str, Position, Direction]]:
    return [
        (board[y + _y][x + _x], (y + _y, x + _x), (_y, _x))
        for _y in range(0, 3, 2)
        for _x in range(0, 3, 2)
        if 0 <= x + _x < len(board[0]) and 0 <= y + _y < len(board) and _x != _y
    ]
...
def is_this_xmas(
    ...
    cross_neighbors = get_cross_neighbors(x, y, board)
